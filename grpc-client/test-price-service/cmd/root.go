...
var client pricepb.PriceServiceClient
var requestCtx context.Context
var requestOpts grpc.DialOption

// rootCmd represents the base command when called without any subcommands
var rootCmd = &cobra.Command{
	Use:   "test-price-service",
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.test-price-service.yaml)")
	...
	conn, err := grpc.Dial("34.78.254.186:50053", requestOpts)
	if err != nil {
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".test-price-service" (without extension).
		viper.AddConfigPath(home)
		viper.SetConfigName(".test-price-service")
	}
